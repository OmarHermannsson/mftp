...
- **Per-chunk hashing**: BLAKE3 is computed over the decompressed chunk bytes and verified on the receiver before the chunk is written. A hash mismatch drops the chunk (leaving it unmarked) rather than aborting; at the completion checkpoint the receiver requests still-missing chunks via in-band incremental repair (protocol v5, `ReceiverMessage::Retransmit`, single-file only) over the control stream, falling back to abort + resume only if repair is unavailable or exhausted.
